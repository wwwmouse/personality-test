...
async function analyze(picks, reasons) {
  ...
  if (!res.ok) {
    return { error: `DeepSeek 出错（${res.status}）：${(await res.text()).slice(0, 120)}` }
  }
  try {
    return { report: JSON.parse((await res.json()).choices[0].message.content) }
  } catch {
...
for (const p of profiles) {
  ...
  console.log(`【${p.name}】${types.join(' / ')} → 众数 ${m}`)
}
