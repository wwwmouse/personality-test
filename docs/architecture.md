...
【Vercel · 房间 2】Serverless 函数 api/analyze.js（vercel.json 把 /api/* 全指到这）
 ...
 │  ③ 验货（不合格退货重写，最多 3 次）④ 记账（写 Blob 账本）
 ▼
...
| DeepSeek | 云上 | 读答案、写报告 | —— |
| 账本 | Vercel Blob（私密仓库） | 记聚合数字（次数/类型/填写率/反馈） | —— |
...
【Express 后端店】3001 端口：server.js 开店、app.js 出菜（和线上同一份菜谱）
 │  读 prompt.md、拿 .env 的 key、调 DeepSeek、验货、记账（记本地文件）
 ▼
...
2. 账本记在本地文件 `.stats-local.json`（线上记 Vercel Blob）
...
| 5 | 检查通过 → 画面切"🤔 分析中"（4 句文案每 4 秒轮播） | `App.vue`：`phase = 'analyzing'` + `analyzingLines` |
...
| 13 | 合格 → 顺手记账：总次数 +1、该类型 +1、理由填写数累加（记账失败绝不拦报告） | `app.js` → `stats-store.js`：`readStats()` / `writeStats()` |
...
**是什么**：统计账本的"仓库管理员"，对外只暴露两个函数：`readStats()` / `writeStats()`。其他代码（app.js）不关心账本存在哪。

**账本长什么样**（只记聚合数字，不存任何原文）：
...
```json
{
  ...
  "reasonFilled": 9,
  "reasonTotal": 240,
  "feedback": { "like": 4, "dislike": 1 }
}
```

**怎么运转**：`storageMode()` 决定账本放哪：
- 有 Blob 令牌/身份牌 → 云端私密仓库 `stats.json`（Vercel 上用 OIDC 自动发短效令牌，没有可泄露的长钥匙）；
- 本地没配 → 本地文件 `.stats-local.json`（开发方便）；
- **在 Vercel 上却都没配 → 宁可报错，也不悄悄写进"睡一觉就丢"的地方**。

**为什么这么设计**：① 隐私红线——只记数字不记答案；② 换仓库只改这一个文件（将来流量大了换 Redis，别处零改动）；③ 记账失败绝不拦报告（用户没做错任何事，悄悄记日志即可）。

**已知短板（诚实记录）**：读→改→写不是原子的，同一瞬间两个测试同时记账可能互相覆盖。练手流量下无所谓；将来流量大了换 Redis（INCR 天然原子）。
...
| 8 | 统计只记聚合数字，账本私密 + OIDC 短效令牌 | 隐私红线（不存答案原文）；没有可泄露的长钥匙 |
| 9 | 记账失败不拦报告 | 用户没做错任何事，统计是后院的账，不能挡前厅的客 |
| 10 | 账本仓库抽象成一个文件（stats-store.js） | 本地文件 ↔ 云端 Blob ↔ 将来 Redis，换仓库只改一个文件 |
...
| 反馈点了没反应 | 浏览器 Network 面板看 `/api/feedback` | 账本仓库配置（Blob 连接） |
